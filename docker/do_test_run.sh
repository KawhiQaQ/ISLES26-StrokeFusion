...
MODEL_DIR="${ISLES26_MODEL_DIR:-${SCRIPT_DIR}/model}"
MODEL_ARCHIVE="${ISLES26_MODEL_ARCHIVE:-${SCRIPT_DIR}/../submission_artifacts/model-postprocess.tar.gz}"
...
STAGING_MODEL_DIR="${SCRIPT_DIR}/test/.staging_model"
...
setup() {
    ...
    if [[ ! -f "${MODEL_DIR}/e1/fold_all/checkpoint_final.pth" || \
          ! -f "${MODEL_DIR}/v15/fold_all/checkpoint_final.pth" ]]; then
        if [[ ! -f "$MODEL_ARCHIVE" ]]; then
            log "Final Model archive not found: $MODEL_ARCHIVE" error
            exit 1
        fi
        rm -rf "$STAGING_MODEL_DIR"
        mkdir -p "$STAGING_MODEL_DIR"
        tar -xzf "$MODEL_ARCHIVE" -C "$STAGING_MODEL_DIR"
        MODEL_DIR="$STAGING_MODEL_DIR"
        log "Staged the frozen Model archive for local testing"
    fi
    ...
    if docker info 2>/dev/null | grep -q "Runtimes:.*nvidia"; then
        ...
        log "NVIDIA container runtime is required by the frozen ensemble" error
        exit 1
    fi
