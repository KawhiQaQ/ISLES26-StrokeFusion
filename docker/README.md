...
contains the frozen E1-SWA + V15-SWA inference path.
...
If `model/e1` and `model/v15` are absent, the test script stages the frozen
Model archive from `../submission_artifacts/model-postprocess.tar.gz` into an
ignored temporary directory. Outputs are written under `test/output/`.
...
`../scripts/package_final_model.sh`. The two files must be uploaded to their
respective Grand Challenge resource types.
